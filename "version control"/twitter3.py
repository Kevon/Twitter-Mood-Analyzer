...
def player(name, text, reply):
    playerMood = mood(text)
    if playerMood == 100.0:
        uApi.update_status("@"+str(name)+" That last tweet of yours ranked a 100... That's extremely positive. Yay!", reply)
    elif playerMood > 75.0:
        uApi.update_status("@"+str(name)+" That last tweet of yours ranked a "+'%.3f' % playerMood+"... That's very positive. Yay!", reply)
    elif playerMood > 50.0:
        uApi.update_status("@"+str(name)+" That last tweet of yours ranked a "+'%.3f' % playerMood+"... That's fairly positive.", reply)
    elif playerMood > 25.0:
        uApi.update_status("@"+str(name)+" That last tweet of yours ranked a "+'%.3f' % playerMood+"... That's slightly positive.", reply)
    elif playerMood > 0.0:
        uApi.update_status("@"+str(name)+" That last tweet of yours ranked a "+'%.3f' % playerMood+"... That's a little positive.", reply)
    elif playerMood == 0.0:
        uApi.update_status("@"+str(name)+" That was a neutral tweet. Like, abnormally neutral. Lacks any and all emotion... (Are you a robot?)", reply)
    elif playerMood == -100.0:
        uApi.update_status("@"+str(name)+" That last tweet of yours ranked a -100... That's extremely negative. (Are you ok?)", reply)
    elif playerMood > -25.0:
        uApi.update_status("@"+str(name)+" That last tweet of yours ranked a "+'%.3f' % playerMood+"... That's a little negative.", reply)
    elif playerMood > -50.0:
        uApi.update_status("@"+str(name)+" That last tweet of yours ranked a "+'%.3f' % playerMood+"... That's slightly negative.", reply)
    elif playerMood > -75.0:
        uApi.update_status("@"+str(name)+" That last tweet of yours ranked a "+'%.3f' % playerMood+"... That's fairly negative.", reply)
    elif playerMood > -100.0:
        uApi.update_status("@"+str(name)+" That last tweet of yours ranked a "+'%.3f' % playerMood+"... That's very negative. (Are you ok?)", reply)
    print name, text, playerMood
