...
handle = deque(maxlen=10)
tweet = deque(maxlen=10)
...
class PStreamListener(PStreamListener):
    def on_status(self, status):
        ...
        try:
            print "Tweet get."
            handle.appendleft(status.author.screen_name)
            tweet.appendleft(status.text)
...
class PublicTweets:
    ...
    def newTweet(self, name, text, mood):
        ...
        self.labelList.append(self.tweet)
        self.labelList.append(self.userName)
        
    def draw(self):
        for label in self.labelList:
            label.draw()
            alpha = label.color
            alpha = alpha[:-1] + (alpha[-1]-4, )
            label.color = alpha
