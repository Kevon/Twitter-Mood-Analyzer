    ...
    def on_status(self, status):
        try:
            if status.author.screen_name not in userNames:
                ...
                playerTweetQ.append(status.text)
                playerIDQ.append(status.author.screen_name)
                #print status.text
...
class PlayerTweets:
    ...
    def newTweet(self, name, text, mood):
        ...
        self.tweet = pyglet.text.Label(text,
                              font_name='Helvetica',
                              font_size=20, bold=True, color=(255, 255, 255, 255),
                              x=self.x, y=self.y, anchor_x='center')

        self.userName = pyglet.text.Label("@"+name,
                              font_name='Helvetica',
                              font_size=14, bold=True, color=(150, 150, 150, 255),
                              x=self.x, y=self.y-20, anchor_x='center')
...
def update(ThisFunctionAlwaysWantsAnArgument):
    ...
    if len(playerTweetQ) != 0:
        playerTweet = playerTweetQ.popleft()
        playerHandle = playerIDQ.popleft()
        playerMood = mood(playerTweet)
        playerTweet.encode('ascii', 'ignore')
        tweetCount = tweetCount + 1

    if tweetCount > 0:
        playerTweets.newTweet(playerHandle, playerTweet, playerMood)
